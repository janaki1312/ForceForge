...
function TaskCard({
  ...
  return (
        <div
          onClick={onStart}
          className="glass-card cursor-pointer rounded-2xl p-4 transition hover:border-primary/40 hover:scale-[1.01]"
        >
      ...
      {t.tag && (
        <div className="mt-2 inline-block rounded-md border border-border bg-card/60 px-2 py-0.5 text-[10px] uppercase tracking-wider text-muted-foreground">
          {t.tag}
        </div>
      )}
...
function Tasks() {
  ...
  useEffect(() => {
    ...
    const converted: Task[] = (plan.execution_plan ?? []).map((task: any, index: number) => ({
      id: index,
      title: task.title,
      deadline: task.deadline,
      estimatedMinutes: task.estimated_minutes,
      priority: task.priority.toLowerCase(),
      risk: plan.risk.level.toLowerCase(),
      progress: 0,
      status: "todo",
      tag: task.priority,
      group: index === 0 ? "today" : index <= 2 ? "tomorrow" : "this_week",
    }));
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Everything on your plate, grouped by when it needs to move.
          </p>
        </div>
